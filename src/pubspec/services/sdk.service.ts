...
import { type LatestSDKModel, sdkModelSchema } from "../data/sdk.model";
...
export class SDKService {
  ...
  async getDartAndFlutterSDKVersions(): Promise<LatestSDKModel> {
    ...
    try {
      ...
      throw new Error(`Invalid JSON in SDK versions response: ${error}`);
    }
    ...
    const latestReleaseCandidate = parsedBody.data.releases.find(
      (item) => item.hash === latestStableHash && item.dart_sdk_version !== undefined,
    );
